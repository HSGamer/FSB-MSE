...
return_date_entry = tk.Entry(return_date_frame, state="disabled")
...
quit_button = tk.Button(button_frame, text="Quit", command=app.destroy, bg="lightcoral")
...
def save_info():
    ...
    return_date = return_date_entry.get() if return_var.get() else None
    ...
    try:
        ...
        messagebox.showerror("Error", "From Date is not in the correct format (dd/mm/yyyy).")
        return

    if from_date_object < datetime.now():
        messagebox.showerror("Error", "From date must be in the future.")
        return

    if return_var.get():
        try:
            return_date_object = datetime.strptime(return_date, "%d/%m/%Y")
        except ValueError:
            messagebox.showerror("Error", "Return Date is not in the correct format (dd/mm/yyyy).")
            return

        if from_date_object >= return_date_object:
